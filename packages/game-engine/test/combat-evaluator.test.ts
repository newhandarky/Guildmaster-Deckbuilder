...
describe('generic combat evaluation pipeline', () => {
  ...
  it('keeps combat evaluation facts compatible with post-command choice continuation', () => {
    ...
    const mismatchedRegistry = structuredClone(serializeSnapshot(suspended.state)); mismatchedRegistry.state.effectState.pendingPostCommand!.facts[0]!.payload!.evaluation.registry.modules[1]!.version = 'tampered'; mismatchedRegistry.state.effectState.pendingPostCommand!.events.find(({ type }) => type === 'COMBAT_EVALUATED')!.payload!.evaluation.registry.modules[1]!.version = 'tampered';
