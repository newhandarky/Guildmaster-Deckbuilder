...
export function evaluateTeamOverflow(state: GameState, ruleset: Ruleset, input: TeamOverflowInput): TeamOverflowResult {
  ...
  const policy = ordered[0]!; const candidateIds = policy.mode === 'discard-oldest' ? player.party.slice(0, overflowCount).map((slot) => slot.adventurerId) : [input.incomingMemberId, ...player.party.slice().reverse().slice(0, Math.max(0, overflowCount - 1)).map((slot) => slot.adventurerId)];
