...
export function evaluateDiceRoll(state: GameState, ruleset: Ruleset, input: DiceRollInput): DiceRollResult {
  ...
  if (JSON.stringify(input.registry) !== JSON.stringify(registry) || JSON.stringify(state.rulesModules.map(({ id, version }) => ({ id, version }))) !== JSON.stringify(registry.modules)) return { status: 'failed', reason: 'REGISTRY_VERSION_MISMATCH', error: 'Dice registry fingerprint mismatch.' };
