...
export function evaluateEnemyTargetDamage(state: GameState, ruleset: Ruleset, request: EnemyTargetDamageRequest): EnemyTargetDamageResult {
  ...
  const lethalOutcome = request.lethalOutcome ?? 'defeated';
  const resolution = lethal ? evaluateEnemyTargetResolution(state, ruleset, { schemaVersion: 1, targetId: request.targetId, outcome: lethalOutcome, ...(request.policy ? { policy: request.policy } : {}) }) : undefined;
  if (resolution && resolution.status !== 'ready') return resolution;
  return { status: 'ready', evaluation: { schemaVersion: 1, input: { schemaVersion: 1, encounter: { encounterId: valid.value.encounter.encounterId, encounterKind: valid.value.encounter.kind }, target: targetRef(canonical), fixedTargetIds: [...valid.value.encounter.targetIds], fixedAttachmentIds: [...canonical.attachments], healthBefore: { ...canonical.health }, requestedDamage: request.requestedDamage, lethalOutcome, policy: { moduleId: valid.value.policy.moduleId, policyId: valid.value.policy.policyId }, registry: valid.value.registry }, healthAfter, actualDamage, lethal, terminalBefore: false, terminalAfter: lethal, ...(resolution?.status === 'ready' ? { resolution: resolution.evaluation } : {}), reasonCode: { ...valid.value.policy.reasonCode } } };
}
