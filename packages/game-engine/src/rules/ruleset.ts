...
export function createRuleset(packs: readonly ContentPack[], modules: readonly RulesModule[], options?: ContentRegistryOptions): Ruleset {
  ...
  for (const module of composedModules) {
    ...
    for (const rule of module.bondConditionRules ?? []) { const errors = validateBondConditionRule(rule, module.id); const ref = `${module.id}\u0000${rule.ruleId}`; if (bondRefs.has(ref)) errors.push(`Duplicate bond condition rule: ${module.id}/${rule.ruleId}.`); if (!registry.bonds.some(({ id }) => id === rule.bondId)) errors.push(`Bond condition rule ${rule.ruleId} references unknown bond ${rule.bondId}.`); const timing = rule.completionTiming ?? 'state'; const existingTiming = bondTimings.get(rule.bondId); if (existingTiming && existingTiming !== timing) errors.push(`Bond ${rule.bondId} has conflicting completion timing policies.`); bondRefs.add(ref); bondTimings.set(rule.bondId, timing); if (errors.length) throw new Error(errors.join(' ')); }
