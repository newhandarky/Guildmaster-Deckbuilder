...
export function createRuleset(packs: readonly ContentPack[], modules: readonly RulesModule[], options?: ContentRegistryOptions): Ruleset {
  ...
  for (const module of composedModules) {
    ...
    for (const policy of module.teamCapacityEnforcementPolicies ?? []) { const errors = validateTeamCapacityEnforcementPolicy(policy, module.id); const ref = `${module.id}\u0000${policy.policyId}`; if (capacityRefs.has(ref)) errors.push(`Duplicate team capacity enforcement policy: ${module.id}/${policy.policyId}.`); if (capacityPriorities.has(policy.priority)) errors.push(`Team capacity enforcement policy priority ${policy.priority} is ambiguous.`); capacityRefs.add(ref); capacityPriorities.add(policy.priority); if (errors.length) throw new Error(errors.join(' ')); }
