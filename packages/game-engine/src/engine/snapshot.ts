...
export function restoreSnapshot(snapshot: unknown): GameState {
  ...
  if (command) {
    ...
    if (command.kind === 'combat-reward') {
      const rollbackState = GameStateSchema.parse(command.rollbackState) as GameState; const registry = { rulesetVersion: state.rulesetVersion, modules: state.rulesModules.map(({ id, version }) => ({ id, version })) }; const evaluation = command.evaluation;
