...
export function settleRest(state: GameState, ruleset: Ruleset, player: PlayerState, events: DomainEvent[], commandId: string): EngineError | undefined {
  ...
  for (const cardId of [...player.hand, ...player.playArea]) pushDiscard(state, ruleset, player.id, cardId); player.hand = []; player.playArea = [];
  player.turnPurchaseBonus = 0; player.turnPurchaseSpent = 0; player.turnCombatBonus = 0; player.turnMarketRefreshed = false;
  try {
    refillConfiguredSupplyRows(state, ruleset, events); attachTargets(state, ruleset); drawCards(state, player.id, handSize.evaluation.effectiveHandSize, events);
  } catch (error) {
