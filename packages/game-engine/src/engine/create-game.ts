...
export function createGame(config: CreateGameConfig, ruleset: Ruleset): GameState {
  ...
  const bossCount = ruleset.modules.reduce((count, module) => module.getBossSetupCount?.(config.players.length, count) ?? count, config.players.length + 2);
  if (!Number.isInteger(bossCount) || bossCount < 1) throw new Error(`Rules Modules produced an invalid boss setup count: ${bossCount}.`);
