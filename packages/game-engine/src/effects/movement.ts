...
export type MoveSuccess = { ok: true; from: EngineCardLocation; to: EngineCardLocation };
export type MoveResult = MoveFailure | MoveSuccess;
export type MoveRequest = { cardInstanceId: string; from: EngineCardLocation; to: EngineCardLocation; actorId: string; context: EffectContext; registry: ContentRegistry; position?: 'top' | 'bottom' | number; permission?: 'controller-only' | 'system'; transferOwnership?: boolean };
...
export function resolveLocation(location: EngineCardLocation, context: EffectContext): EngineCardLocation | undefined {
...
function sameLocation(left: EngineCardLocation, right: EngineCardLocation): boolean { return JSON.stringify(left) === JSON.stringify(right); }
export function isCardAtLocation(state: GameState, location: EngineCardLocation, cardId: string): boolean {
...
function remove(state: GameState, location: EngineCardLocation, cardId: string): void {
...
function insert(state: GameState, location: EngineCardLocation, cardId: string, position: MoveRequest['position']): void {
...
function ownerOf(location: EngineCardLocation): string | undefined { return location.kind === 'player-zone' || location.kind === 'party' || location.kind === 'equipment' ? (location.player as { playerId: string }).playerId : undefined; }
...
export function moveCard(state: GameState, request: MoveRequest): MoveResult {
  ...
  let from: EngineCardLocation; let to: EngineCardLocation;
