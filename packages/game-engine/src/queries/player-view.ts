...
export function projectPlayerView(state: GameState, ruleset: Ruleset, viewerId: string): PlayerView {
  ...
  const visibleIds = new Set([...visibleSelf.hand, ...visibleSelf.discardPile, ...visibleSelf.playArea, ...visibleSelf.party.flatMap((slot) => [slot.adventurerId, ...(slot.equipmentId ? [slot.equipmentId] : [])]), ...[...opponentParties.values()].flatMap((party) => party.flatMap((slot) => [slot.adventurerId, ...(slot.equipmentId ? [slot.equipmentId] : [])])), ...Object.values(visibleZones).flatMap((zone) => zone.cardIds), ...visibleTargetIds]);
