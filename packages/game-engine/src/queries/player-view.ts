...
import { getPurchasePower } from './legal-commands.js';
...
export function projectPlayerView(state: GameState, ruleset: Ruleset, viewerId: string): PlayerView {
  ...
  const selfParty = projectPublicParty(state, ruleset, player);
  ...
  const departureCommand = state.effectState.pendingCommand?.kind === 'combat-departure-choice' ? state.effectState.pendingCommand : undefined;
  const decisionOptions = choice?.order ? choice.order.cardIds.map((id) => ({ id, cardId: id, definitionId: state.cards[id]!.definitionId })) : choice?.options.map(({ id }) => {
    const selectedCardIds = departureCommand?.optionCandidateIds[id]?.map((candidateId) => departureCommand.candidates.find((candidate) => candidate.candidateId === candidateId)?.adventurerId).filter((cardId): cardId is string => Boolean(cardId));
    return { id, ...(state.cards[id] ? { cardId: id, definitionId: state.cards[id]!.definitionId } : {}), ...(selectedCardIds ? { selectedCardIds, selectedDefinitionIds: selectedCardIds.map((cardId) => state.cards[cardId]!.definitionId) } : {}) };
  }) ?? [];
  ...
  return {
    ...
    self: { ...visibleSelf, party: selfParty, drawPileCount: drawPile.length }, partyLimit: getPartyLimit(ruleset, state, player),
    availablePurchasePower: getPurchasePower(state, ruleset, player.id), bondEvaluations,
    ...
    ...(choice?.actorId === viewerId && kind ? { decisionPrompt: { schemaVersion: 1 as const, decisionKind: kind, choiceId: choice.choiceId, minSelections: 1, maxSelections: 1, options: decisionOptions, ...(choice.order ? { order: { kind: choice.order.kind ?? 'player-deck-top' as const, cardIds: [...choice.order.cardIds], mayRemove: choice.order.mayRemove } } : {}) } } : {}),
    ...
    zones: structuredClone(visibleZones), enemyTargets: publicTargets, cards, removedCardCount: state.removedCards.length, ...(state.endState ? { endState: structuredClone(state.endState) } : {})
  };
}
