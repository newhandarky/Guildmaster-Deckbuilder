...
export type CpuReasonCode =
  ...
  | 'RESPOND_REQUIRED_CONSENT' | 'BLOCKED_UNSUPPORTED_DECISION';
...
export function decideCpuAction(context: CpuDecisionContext): CpuDecisionResult {
  ...
  if (context.legalCommands.some(({ type }) => type === 'RESOLVE_EFFECT_CHOICE')) return { status: 'blocked', reasonCode: 'UNSUPPORTED_DECISION_KIND', diagnostic: 'Effect choice has no typed PlayerDecisionPrompt; CPU stopped without guessing.' };
