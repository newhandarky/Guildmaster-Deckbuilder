...
const starters: readonly ProvisionalCardCandidate[] = [
  ['adventurer-01', '麥娜', 1], ['adventurer-02', '慕莎', 2], ['adventurer-03', '卡儂', 1], ['adventurer-04', '修爾蒂', 1], ['adventurer-05', '辛芙妮', 1]
].map(([suffix, name, combat], index) => card(`base:starter/${suffix}`, 'starter', name as string, 'starter', `card-07.jpg；起始冒險者第 ${index + 1} 張`, [field('cardType', 'adventurer', 'rulebook', 'page-05.jpg；印刷頁 3；玩家設置第 1 點'), field('copies', 1, 'rulebook', 'page-05.jpg；印刷頁 3；玩家設置第 1 點'), field('combat', combat as number, 'starter', `card-07.jpg；起始冒險者第 ${index + 1} 張`), field('setup', 'face-up random initial party', 'rulebook', 'page-05.jpg；印刷頁 3；玩家設置第 1 點'), effect('starter', `card-07.jpg；起始冒險者第 ${index + 1} 張`)]));
...
const starterResources: readonly ProvisionalCardCandidate[] = [
  ...
  card('base:starter/spirit-crystal', 'starter', '精靈結晶', 'starter', 'card-07.jpg；下排右側', [field('cardType', 'starter-resource', 'rulebook', 'page-05.jpg；印刷頁 3；玩家設置第 4 點'), field('copies', 1, 'rulebook', 'page-05.jpg；印刷頁 3；玩家設置第 4 點'), field('honor', 1, 'starter', 'card-07.jpg；下排右側'), field('setup', 'initial hand', 'rulebook', 'page-05.jpg；印刷頁 3；玩家設置第 4 點')])
];
...
const adventurerRows: readonly [string, string, number, number, number, string?][] = [
  ...
  ['adventurer-16','神樂',4,3,2,'進入隊伍時展示牌庫頂；若為道具或裝備則置入手牌。'], ['adventurer-17','蕾普莉絲',3,1,1,'進入隊伍時抽 3 張，然後從手牌棄 1 張。'], ['adventurer-18','羅絲瑪莉',4,2,2,'討伐結束後，若本回合擊敗目標，獲得購買力 2。'], ['adventurer-19','賽席莉亞',4,2,2], ['adventurer-20','費歐娜',3,5,1,'戰力扣除隊伍中其他冒險者數量。'],
  ['adventurer-21','阿爾梅斯',4,3,2,'因戰鬥將棄至棄牌堆時，可改置於牌庫頂。'], ['adventurer-22','露希艾拉',3,1,1], ['adventurer-23','拉菲娜',3,1,1,'討伐開始擲骰；指定場上目標，本回合其戰力降低骰值的一半（向上取整）。'], ['adventurer-24','索娜莉亞',4,1,1,'討伐目標為魔物時，戰力 +3。'], ['adventurer-25','米莉安',4,2,2,'可作為裝備配戴給其他冒險者；配戴者戰力 +2。'],
...
const resourceRows: readonly [string, string, 'item' | 'equipment', number, string][] = [
  ['resource-01','特大治癒藥水','item',3,'從棄牌堆取回 1 張冒險者至手牌。'], ['resource-02','火焰拳套','equipment',3,'若由近戰職業配戴，額外增加戰力。'], ['resource-03','邪魅法典','equipment',3,'若由輔助職業配戴，額外增加戰力。'], ['resource-04','驅邪聖水','item',2,'從手牌棄 1 張魔王後，抽 3 張。'], ['resource-05','維修道具包','item',3,'從棄牌堆取回 1 張裝備至手牌。'],
  ['resource-06','貓咪娃娃','item',1,'購買後將這張牌置入右手邊玩家棄牌堆；該卡被棄置時，置入右手邊玩家棄牌堆。'], ['resource-07','透視眼鏡','equipment',3,'若由遠程職業配戴，額外增加戰力。'], ['resource-08','櫻花果子','item',4,'抽 2 張牌。'], ['resource-09','詛咒之槍','equipment',5,'若討伐的是魔物，增加戰力。'], ['resource-10','大號梳毛梳','item',3,'棄 1 張手牌後，抽 2 張。'],
  ['resource-11','寫滿的行程表','equipment',6,'其他隊友增加戰力。'], ['resource-12','真龍斧連枷','equipment',5,'裝備者因戰鬥棄牌時，移除此裝備。'], ['resource-13','賢者之石','item',3,'從棄牌堆取回 1 張非賢者之石的法師卡至手牌。'], ['resource-14','絲綢緞帶','equipment',3,'限特定職業配戴；配戴者加戰力，且其因戰鬥棄牌時移除此裝備。'], ['resource-15','魔法除塵撢','item',4,'從手牌、隊伍或棄牌堆移除 1 張牌。'],
  ['resource-16','鬼哭太刀','equipment',6,'限坦克或近戰配戴；討伐時棄 1 張魔王或魔物手牌，戰力加該卡購買力。'], ['resource-17','金色水晶球','item',4,'抽 3 張後，從手牌棄 1 張。'], ['resource-18','名貴的首飾','equipment',4,'擊敗目標後，裝備者仍在隊伍時抽 1 張。'], ['resource-19','靈能法杖','equipment',4,'限魔法或輔助配戴；討伐開始棄任意數量手牌，戰力增加等同棄牌數。'], ['resource-20','充能魔劍','equipment',5,'限近戰配戴；額外增加戰力。'],
...
const monsterRewards: Record<string, string> = {
  ...
  'monster-07': '獲得商店 1 張費用小於等於 3 的遠程或輔助物資，置入棄牌堆。',
  'monster-08': '獲得商店 1 張費用小於等於 4 的遠程或輔助物資，置入棄牌堆。',
  'monster-09': '抽 2 張牌。',
...
const bossEffects: Record<string, string> = {
  'boss-01': '戰力增加商店裡裝備的數量；獲得購買力 5，並獲得商店 2 張費用小於等於 4 的遠程或輔助物資，置入棄牌堆。',
  ...
  'boss-05': '討伐此魔王時所有裝備失效。獲得購買力 5，獲得商店 2 張費用小於等於 3 的遠程或輔助物資，置入棄牌堆。',
  ...
  'boss-11': '僅能用 1 張冒險者卡進行討伐。獲得購買力 5，獲得商店 2 張費用小於等於 3 的遠程物資，置入棄牌堆。'
};
...
const bondRows: readonly [string, number, string][] = [
  ...
  ['順手牽羊',3,'一回合內招募冒險者與購買遠程或裝備物資各一次。'], ['援護射擊',4,'討伐階段開始時，隊伍最後有 2 位遠程。'], ['颯爽登場',4,'一回合內把 3 位非基礎冒險者加入隊伍。'], ['恐懼凝視',4,'討伐階段開始時，隊伍中至少有 3 位坦克或近戰。'], ['野性狂化',4,'討伐階段開始時，隊伍最前端有 2 位近戰。'],
  ['急速施法',5,'擊敗魔物或魔王後，隊伍內有 2 位以上法師。'], ['肉身強化',5,'擊敗魔物或魔王後，隊伍內有 2 位以上坦克。'], ['全力以赴',4,'一個行動階段中使用 3 張以上遠程物資。'], ['情熱舞蹈',6,'討伐階段開始時，隊伍中有 5 種不同職業的冒險者各一位，其中至少 3 位為非基礎冒險者。'], ['天馬行空',5,'擊敗魔物或魔王後，隊伍內有 2 位以上遠程。'],
  ...
  ['靈光一閃',5,'隊伍中有 3 位以上同職業的冒險者。'], ['精神陶亂',4,'討伐階段開始時，隊伍最前端有 2 位坦克。'], ['冒冒失失',6,'一回合內有 3 張以上裝備隨冒險者一同棄至棄牌堆。'], ['全場鎮壓',3,'擊敗 1 隻魔物。'], ['母性感化',4,'一回合內使用 3 張以上道具進行購買。'],
...
const helperEffects: readonly string[] = [
  ...
  '此協助者進場、離場時，該回合玩家選擇冒險者／物資牌庫之一，從該牌庫翻開等同玩家數量的牌；從當回合玩家開始順時針，每位玩家從中選擇 1 張加入手牌。'
];
