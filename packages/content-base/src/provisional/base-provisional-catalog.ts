...
const resources = resourceRows.map(([id, name, type, cost, text], i) => {
  ...
  const candidate = card(`base:resource/${id}`, 'resource', name, 'resource', region, [field('cardType', type, 'rulebook', 'page-06.jpg；印刷頁 4；物資卡介紹'), field('copies', undefined, 'rulebook', 'page-04.jpg；印刷頁 2；物資卡共 59 張；card-03.jpg 顯示 28 種', 'medium', 'exception', 'Per-card multiplicities cannot be derived safely from the total and one visual sheet.'), ...stats('resource', region, cost), effectField, field('effectTiming', type === 'item' ? 'action; discard at rest after use' : 'equip during action; remains until equipped adventurer leaves party', 'rulebook', 'page-06.jpg；印刷頁 4；物資卡介紹', 'high') ]);
  // A profession symbol printed in resource text is a cross-card affinity,
  // not an adventurer profession. Resource 13 explicitly excludes its own
  // mage-affinity definition; resource 19 is the other audited mage match.
