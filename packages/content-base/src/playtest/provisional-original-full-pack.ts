...
const enabledFoundationIds = new Set(['resource-01', 'resource-02', 'resource-03', 'resource-04', 'resource-05', 'resource-07', 'resource-08', 'resource-10', 'resource-13', 'resource-15', 'resource-17', 'resource-18', 'resource-25', 'resource-27']);
const enabledAdventurerRuleIds = new Set(['adventurer-02', 'adventurer-09']);
...
export const baseProvisionalOriginalFullContentPack: ContentPack = {
  manifest: { id: 'base:provisional-original-full', version: '0.4.0', hash: 'base-provisional-original-full-v5-audited-stats', role: 'base', contentStatus: 'provisional-playtest' },
...
export const baseProvisionalOriginalFullCapabilityRegistry = {
  engineCapabilities: ['effect-ast', 'snapshot-continuation', 'combat-reward-policy', 'dice-roll', 'equipment-eligibility', 'equipment-combat-modifier', 'equipment-event-lifecycle', 'typed-player-view-choice'] as const,
  ...
  testIds: ['content:provisional-foundation-pack', 'content:provisional-original-full-roster', 'content:provisional-original-full-rules', 'engine:card-use-effect', 'engine:combat-reward-policy', 'engine:dice-roll', 'engine:equipment-eligibility', 'engine:equipment-combat-modifier', 'engine:post-command-pipeline', 'engine:bond-setup', 'cpu:deterministic-choice', 'cpu:deterministic-legal-scoring', 'cpu:keep-bonds'] as const,
};
...
const definitionCapabilities = baseProvisionalOriginalFullContentPack.definitions.map((definition): FullProvisionalCapabilityEntry => {
  ...
  const usesCombatRewardPolicy = definition.type === 'monster' && enabledMonsterRewardIds.has(definition.id.split('/').at(-1) ?? '');
  ...
  const requiredCapabilities = enabled
    ? ['snapshot-continuation', ...(usesCombatRewardPolicy ? ['effect-ast', 'combat-reward-policy'] : []), ...(usesDice ? ['dice-roll'] : []), ...(usesEquipmentEligibility ? ['equipment-eligibility'] : []), ...(usesEquipmentCombatModifier ? ['equipment-combat-modifier'] : []), ...(!usesCombatRewardPolicy && !usesEquipmentEligibility && !usesEquipmentCombatModifier ? ['effect-ast'] : []), ...(definition.equipmentEventTriggers ? ['equipment-event-lifecycle'] : []), ...(hasChoice ? ['typed-player-view-choice'] : [])]
    : [];
  return {
    ...
    evidenceReference: usesCombatRewardPolicy || usesEquipmentEligibility || usesEquipmentCombatModifier ? `visual-high-confidence:${definition.id}` : enabled ? `foundation-audit:${definition.id}` : `visual-roster:${definition.id}`,
    ...
    testIds: enabled ? [...new Set([usesCombatRewardPolicy ? 'engine:combat-reward-policy' : 'content:provisional-foundation-pack', ...(usesEquipmentEligibility ? ['engine:equipment-eligibility'] : []), ...(usesEquipmentCombatModifier ? ['engine:equipment-combat-modifier'] : []), definition.equipmentEventTriggers ? 'engine:post-command-pipeline' : usesCombatRewardPolicy || usesEquipmentEligibility || usesEquipmentCombatModifier ? 'content:provisional-original-full-rules' : 'engine:card-use-effect', ...(usesDice ? ['engine:dice-roll'] : []), hasChoice ? 'cpu:deterministic-choice' : 'cpu:deterministic-legal-scoring'])] : ['content:provisional-original-full-roster'],
