...
module state 必須是 JSON 可序列化資料；lifecycle hook 與 combat rule 不得持有 closure 或直接寫入 `GameState`，所有變更只由 engine 解讀版本化資料後產生。continuous hook 目前只建立可驗證 evaluation boundary，尚未執行卡牌持續效果。卡牌內容、卡牌專屬討伐獎勵 policy、裝備資格與供應列泛用設定仍未實作。
