...
module state 必須是 JSON 可序列化資料；lifecycle hook、combat rule 與 equipment eligibility rule 不得持有 closure 或直接寫入 `GameState`，所有變更只由 engine 解讀版本化資料後產生。continuous hook 目前只建立可驗證 evaluation boundary，尚未執行卡牌持續效果。泛用裝備資格 evaluation 已實作，但未載入任何卡牌或裝備內容；卡牌專屬討伐獎勵 policy、特定裝備效果與供應列泛用設定仍未實作。
...
目前 reducer 會為成功 command 與其產生的 facts 發出 `command-before`、`event-before`、`event-after`、`command-after` boundary；所有 boundary 均可透過可序列化 continuation 暫停，整批仍由 command transaction 保護。中性 `grant-combat-reward` primitive 可表達抽牌、暫時戰力／購買力與 counter 獎勵，但尚未掛接任何卡牌資料。
