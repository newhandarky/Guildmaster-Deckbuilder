...
module state 必須是 JSON 可序列化資料；lifecycle hook、combat rule、combat reward policy 與 equipment eligibility rule 不得持有 closure 或直接寫入 `GameState`，所有變更只由 engine 解讀版本化資料後產生。泛用 CombatRewardPolicy evaluation 已實作，defeat dispatch 與 preview 共用固定 target／encounter evaluation；未載入任何正式或 provisional 獎勵 policy。
