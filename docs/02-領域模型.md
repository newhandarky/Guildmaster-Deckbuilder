...
| `effectState` | pending choice 與 pending lifecycle dispatch 的可序列化 continuation |
...
`effectState.pendingCommand` 保存被 command-before choice 暫停的原始 `CommandEnvelope`。choice 完成後只接續未完成 hook，再執行一次原 command 與其 event/command-after boundaries；失敗回到 command 起點。
