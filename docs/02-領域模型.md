...
module state 必須是 JSON 可序列化資料；lifecycle hook 不得持有 closure 或直接寫入 `GameState`，所有變更只由 engine 解讀 Effect AST 後產生。continuous hook 目前只建立可驗證 evaluation boundary，尚未執行卡牌持續效果。卡牌內容、卡牌專屬討伐獎勵 policy、裝備資格與供應列泛用設定仍未實作。

目前 reducer 會為成功 command 與其產生的 facts 發出 `command-before`、`event-before`、`event-after`、`command-after` boundary；整批仍由 command transaction 保護。`command-before` 不可 suspend，直到 protocol 有可序列化的原 command continuation。中性 `grant-combat-reward` primitive 可表達抽牌、暫時戰力／購買力與 counter 獎勵，但尚未掛接任何卡牌資料。
