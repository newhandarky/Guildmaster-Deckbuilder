...
目前已建立 `base:provisional-foundation` 內部垂直切片：7 個起始定義、8 種冒險者、7 種物資、4 種魔物與 4 張魔王，共 30 個中性名稱定義。此切片的數值來自候選 catalog；物資逐種類張數仍缺乏來源證據，因此目前各採 2 張作為 pack 自有的數位 playtest 組成，不宣稱等同官方實體配比。它必須由遠征入口明確選擇，並由 engine `allowProvisionalPlaytest` 閘門授權；存檔／Replay 使用 `0.4.0` 的獨立 manifest 指紋，不能與 Demo 或舊 foundation 存檔混用。

這仍不是完整基礎版 playtest Content Pack。候選物資 01、04、05、08、10、17 標記為 `playtest:effect-enabled`：01／05 依卡種從棄牌堆取回冒險者／裝備，04 棄 1 張手牌中的魔王後抽 3 張，08 執行「抽 2 張牌」，10 執行「棄 1 張手牌後抽 2 張」，17 執行「抽 3 張後棄 1 張手牌」。動態選牌由 authoritative player zone 與 versioned predicate 產生合法選項，並以版本化 card-use continuation 保存 root Command、回滾點、事件與 Effect context；可跨 Snapshot／Replay 恢復，完成時只提交一次 revision。沒有符合 predicate 的候選時，legal Commands 不會列出該道具。候選物資 02 已接入購買／配戴流程，但卡面效果仍標記 `playtest:effects-disabled`。UI 會持續顯示其餘效果未啟用；完整裝配仍需協助者 adapter、個別時序與第二人覆核。production 預設仍是原創 Demo，provisional 不得自動載入。
