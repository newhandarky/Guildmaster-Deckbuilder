...
目前已建立 `base:provisional-foundation` 內部垂直切片：7 個起始定義、8 種冒險者、11 種物資、4 種魔物與 4 張魔王，共 34 個中性名稱定義。此切片的數值來自候選 catalog；物資逐種類張數仍缺乏來源證據，因此目前各採 2 張作為 pack 自有的數位 playtest 組成，不宣稱等同官方實體配比。它必須由遠征入口明確選擇，並由 engine `allowProvisionalPlaytest` 閘門授權；存檔／Replay 使用 `0.9.0` 的獨立 manifest 指紋，不能與 Demo 或舊 foundation 存檔混用。
