...
已實作資料契約與純 reducer primitives：`sequence`、`conditional`、`choice`、`random`、`move-card`、`draw`、`discard-card`、`remove-from-game`、`modify-value`。choice 形成可序列化 pending state，random 使用 `rngState`；移動會驗證來源、owner-only shared zone、隊伍／裝備關係、卡牌類型與 module zone。尚未實作 trigger registry、持續效果與 replacement ordering；任何需要未決順序的效果都必須拒絕／pending，不能默默套用 active player 或陣列順序。
