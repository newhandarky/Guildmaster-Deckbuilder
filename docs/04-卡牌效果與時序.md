...
已實作資料契約與純 reducer primitives：`sequence`、`conditional`、`choice`、`random`、`move-card`、`draw`、`discard-card`、`remove-from-game`、`modify-value`。choice 形成可序列化 pending state，random 使用 `rngState`；移動會驗證來源、owner-only shared zone、隊伍／裝備關係、卡牌類型與 module zone。lifecycle registry、replacement／event ordering、command-before 與 post-command continuation、continuous runtime、supply refresh 和 multi-target encounter 都已由 JSON-only Rules Module registry 驅動；未決的同 priority 順序仍必須拒絕，不能猜測陣列順序。
