...
目前進度：已交付 opt-in 的 provisional foundation 垂直切片，包含 7 個起始定義、8 種冒險者、4 種物資、4 種魔物與 4 張魔王，共 27 個中性名稱候選；使用獨立 Content Pack 指紋與 `allowProvisionalPlaytest` 閘門。候選物資 08 已接入「抽 2 張牌」，10／17 已接入含動態手牌選擇的棄牌與抽牌效果；三者均通過購買、使用、Snapshot 與 Replay 回歸，選牌期間以版本化 card-use continuation 保存完整 root transaction。候選物資 02 可測試購買／配戴，但卡面效果仍停用。其餘未實作卡面繼續明示 `playtest:effects-disabled`，因此 M3 尚未完成。
