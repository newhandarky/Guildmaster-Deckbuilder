...
目前進度：已交付 opt-in 的 provisional foundation 垂直切片，包含 7 個起始定義、8 種冒險者、11 種物資、4 種魔物與 4 張魔王，共 34 個中性名稱候選；使用獨立 Content Pack 指紋與 `allowProvisionalPlaytest` 閘門。候選物資 01／05 已接入依卡種從棄牌堆取回卡片，04 已接入棄魔王後抽牌，08 已接入「抽 2 張牌」，10／17 已接入含動態手牌選擇的棄牌與抽牌，13 已接入「取回非同名道具卡」，15 已接入跨手牌／隊伍／棄牌堆選擇並移除，18 已接入內容擁有、逐裝備實例排序的擊敗後抽牌，27 已接入「依目前隊伍職業種類抽牌」；十者均通過 definition、authoritative dispatch 與 Snapshot 基礎回歸，共用 Replay pipeline 另由固定跨能力 fixtures 驗證，不宣稱每張候選卡都有獨立 Replay journey。`choose-card` predicate、多可見來源與選中卡片的 canonical source location 由 legal Commands、authoritative executor 與 Snapshot restore 共用；resource-18 只掃描 event actor 仍在隊伍的裝備。equipment trigger schema v1 明定其早於 Rules Module hooks，裝備實例依 party slot 排序，同 definition 多 trigger 的 priority 只在裝備 domain 內生效；來源 context 固定綁定，engine 不含候選卡 ID。resource-13 以 definition type `item` 辨識道具，並依 definition ID 排除所有同名物資。Effect 數值可用 JSON-only `party-distinct-tag-count` 計算隊伍不同職業，同職業不重複計數；五張起始冒險者也已有可稽核職業 tag。沒有合法候選時不會暴露可使用指令。foundation 切片中的候選物資 02 仍只測試購買／配戴；完整四人 Provisional 模式則由獨立 Rules Module 啟用物資 02／03／07／25 的職業相符戰力 +1。其餘未實作卡面繼續明示 `playtest:effects-disabled`，因此 M3 尚未完成。
...
- 協助者垂直切片與 Batch A 已完成：獨立 provisional expansion pack、seed-deterministic setup contribution、hidden deck／public active／retired zones、Boss-only lifecycle 輪替、Snapshot／Replay identity、Web opt-in 與中性 Presentation；目前啟用候選協助者 01／06／07／08／09，其餘七張效果仍停用。
