...
- generic CombatRewardPolicy runtime 與 choice continuation 已實作；preview 與 authoritative defeat 共用 evaluation，reward transaction 保存完整 resolution transcript，可由 command 起點跨 Snapshot canonical replay，且不重複 reducer、events 或 RNG。完整 Provisional 四人模式已由獨立 `base:provisional-original-full-rules` 模組載入候選魔物 01（可選購買力 +1）、02（可選擲六面骰並取得骰面一半無條件進位的購買力）、09（可選抽 2）與 14（可選抽 1）的第一批高信心獎勵。
- 基礎討伐 target、前綴隊伍戰力、道具使用區／休息清理、單裝備欄位。
- `choose-card` 可組合手牌、隊伍、棄牌堆等同玩家可見來源，候選選項保存 canonical source location 並由 Snapshot restore 重算；可宣告穩定的 `skipOptionId`，使可選效果在有候選或零候選時都能明確略過。候選物資 15 與魔物 03／06／10／11 已使用此能力，移除有裝備的隊員時依明確 policy 將裝備置入棄牌堆。
...
- JSON-only equipment combat modifier 可依裝備與配戴者 definition／tag、phase 或 counter 修正實際參戰的隊伍前綴戰力；legal query 與 authoritative attack dispatch 共用 evaluator。火焰拳套、邪魅法典、透視眼鏡與騎士之盾已確認職業相符時額外 +1，並在 full Provisional Rules Module 啟用。
...
- reducer 已發出 command/event lifecycle boundaries；中性 `grant-combat-reward` primitive 可表達獎勵資料；目前只載入已列於能力矩陣並具 focused tests 的候選效果。
...
- generic dice roll runtime 已 supported：Rules Module 只註冊 JSON-only sides，Effect AST 以明確 face outcome 執行；DIE_ROLLED payload、Snapshot 與 choice resume 可重播且不重擲。候選魔物 02 已載入六面骰獎勵，其他卡牌骰子內容仍保持停用。
...
6. **進行中 P4：內容接線與 Vol.1。** 已交付 opt-in 的 34 定義 foundation 切片、模式選擇、manifest 存檔恢復與瀏覽器回歸。候選物資 01／05 的棄牌堆卡種取回、04 的棄魔王後抽牌、08 的即時「抽 2」、10／17 的動態選牌、棄牌與抽牌、13 的非同名道具卡取回、15 的跨手牌／隊伍／棄牌堆選擇移除、18 的逐裝備實例擊敗後抽牌，以及 27 的隊伍 distinct-profession 動態抽牌，均完成內容資料到 authoritative dispatch、Snapshot／Replay 與 UI 的接線。完整四人模式另啟用物資 02／03／07／25 的職業相符戰力 +1、冒險者 02 的配戴限制、冒險者 09 的配戴中戰力 +1，以及魔物 01／02／03／06／09／10／11／14 的擊敗獎勵。魔物 03／06／10／11 使用通用 optional dynamic choice；零候選仍能送出 skip，魔物 06 的第二次候選依第一次移除後的權威狀態重算。`choose-card` 支援 JSON-only 的 definition type／ID／tag、all／any／not predicate、多個同玩家可見來源與 canonical source location；resource-18 的 immediate trigger 由卡牌 definition 擁有，event-after 只解析本次行動玩家仍掛在隊伍的裝備，依 party slot／priority 執行，engine 不含候選卡 ID。resource-13 依 definition type `item` 辨識道具，再排除同名 definition。Effect draw count 可用 `party-distinct-tag-count` 從公開隊伍狀態計算不同 tag，同職業只算一次。legal query、executor 與 canonical restore 共用同一候選語意。card-use／combat-reward continuation 已版本化，Snapshot 會驗證 registry、效果程式、authoritative 候選／來源與 transaction events；UI 僅從 PlayerView 與 Legal Commands 顯示可見卡名。羈絆完成已改為玩家送出的任意子集合 `COMPLETE_BONDS` 指令，不再由 Engine 自動完成；實際 30 張羈絆仍須逐張建立 boundary-aware 條件資料才會 enabled。負榮譽已可由內容驗證、計分與 CPU 公開評分承載，貓咪娃娃數值校正為 -1，但其跨玩家棄置替代尚未完成，因此仍停用。只有在卡牌資料、時序與測試封口後才逐批移除 `playtest:effects-disabled`；巫妖、米莉安、多人輪選、完整協助者 lifecycle 與其他魔王維持後續批次。
7. **已完成協助者 Batch A。** 獨立 helper expansion 目前啟用 01／06／07／08／09，分別覆蓋物資折扣、冒險者折扣、休息抽 6、隊伍上限與裝備折扣。購買 evaluator 共用於 legal／preview／dispatch，休息 evaluator 在 mutation 前固定抽牌數；其餘七張只測試揭示與輪替。
