...
`id` 是規則、效果、卡片實例、存檔、Replay、AI 與網路同步的唯一卡片識別。`CardDefinition` 不保存角色名稱、圖片檔名、玩家文案或 asset key；引擎不得讀取這些資料。
...
Presentation Pack（Appearance Pack）是獨立、可替換且不影響規則的顯示資料集合。它以 `CardDefinition.id` 對應 `PresentationEntry`，包含 `displayName`、`portraitAssetKey`、`shortDisplayText`、可選 `variant`／`theme`／`locale`。同一張規則卡可有多套人物名稱與圖片；切換 pack 不可改變 effects、數值、definition ID、合法 Command、AI 決策或對局結果。詳見 `20-presentation-pack-spec.md`。
