...
`game-protocol` 定義版本化 JSON Effect AST；`game-engine/effects` 提供 executor 與原子 card movement。Content Pack 未來只能提供驗證通過的 effect data／extension registry，不能把函式、React、圖片或官方名稱傳入 engine。choice 以 pending state 暫停，random 使用權威 `rngState`；trigger／replacement 沒有已註冊的順序 policy 時，resolver 必須回傳 explicit unsupported，不能猜測。
