...
成功 command 會在同一 authoritative transaction 內發出 command 與 reducer fact 的 lifecycle boundaries。前置 command hook 必須同步完成；後置 hook 可以使用既有 pending choice continuation。事件 hook 可用 `eventType` filter，避免未指定的規則誤套用到所有 facts。
