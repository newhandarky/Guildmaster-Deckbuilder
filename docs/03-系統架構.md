...
`rules/combat-reward-evaluator.ts` 以同一模式提供 CombatRewardPolicy registry。它固定 defeated target、definition、encounter、recipient 與 ordered policy refs，純 evaluator 不改 state/RNG；`attackTarget` 只在 `defeat-target` outcome 以既有 transaction executor 執行 matching Effect AST，`remove-target` 不產生討伐獎勵。未載入任何卡牌獎勵資料。
