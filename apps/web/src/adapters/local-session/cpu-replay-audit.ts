...
export function auditCpuReplay(bundle: ReplayBundle | unknown, ruleset: Ruleset): CpuReplayAuditResult {
  ...
  if (parsed.data.automation.profileId !== baseBalancedCpuProfile.profileId || parsed.data.automation.profileVersion !== baseBalancedCpuProfile.version) return { status: 'failed', diagnostic: 'CPU profile identity mismatch.' };
