...
export function App() {
  ...
  const newExpeditionRef = useRef<HTMLButtonElement>(null);
  const previouslyShowingScoreboard = useRef(Boolean(scoreboard));
  ...
  const submitAndClear = (command: GameCommand) => { setEquipmentCardId(undefined); submit(command); queueMicrotask(() => endPhaseRef.current?.focus()); };

  useEffect(() => {
    const showingScoreboard = Boolean(scoreboard);
    if (showingScoreboard && !previouslyShowingScoreboard.current) newExpeditionRef.current?.focus();
    if (!showingScoreboard && previouslyShowingScoreboard.current) endPhaseRef.current?.focus();
    previouslyShowingScoreboard.current = showingScoreboard;
  }, [scoreboard]);

  if (scoreboard) return <main className="app-shell"><section className="hero"><p className="eyebrow">文字版 MVP</p><h1>遠征結算</h1><p>{view.endState?.conditionId ?? '遊戲結束'}</p></section><section className="scoreboard" aria-labelledby="scoreboard-heading"><h2 id="scoreboard-heading">榮譽排名</h2><ol className="scoreboard-list">{scoreboard.map((row) => <li className="score-row" key={row.playerId}><strong>#{row.rank} {row.name}</strong><span>{row.honor} 榮譽</span><small>魔王 {row.defeatedBosses}／魔物 {row.defeatedMonsters}</small></li>)}</ol></section><button ref={newExpeditionRef} className="primary" type="button" onClick={restart}>開啟新遠征</button></main>;
  ...
  return <main className="app-shell" data-testid="game-app">
    ...
    {view.status === 'pendingOfficialRuling' ? <aside className="warning" role="alert">公共供應牌庫耗盡的基礎版官方結果尚待確認；本局已安全暫停。</aside> : null}
    {view.status === 'finalRound' ? <aside className="warning" data-testid="final-round-notice" role="status">最終輪已觸發；將完成目前輪次後結算。</aside> : null}
    ...
    <section data-testid="hand"><h2>手牌</h2><div className="card-row">{view.self.hand.map((cardId) => {
      ...
      const actionLabel = canPlay ? '加入隊伍' : canUse ? '使用道具' : canEquip ? '選擇配戴目標' : undefined;
      ...
      return <Card key={cardId} testId={`hand-card-${cardId}`} instance={view.cards[cardId]} definition={definition as CardDefinition | undefined} presentation={presentation.resolve(definition?.id ?? cardDefinitions[cardId] ?? '')} actionLabel={actionLabel} onClick={onClick} selected={equipmentCardId === cardId} label={canEquip ? '點選後選隊員' : undefined} />;
    })}</div></section>
    ...
    <section className="controls" aria-label="對局控制"><p id="interaction-hint" data-testid="interaction-hint" role="status" aria-live="polite" aria-atomic="true">{interactionHint}</p>{choiceCommands.length ? <div className="choice-controls" role="group" aria-labelledby="choice-heading"><h2 id="choice-heading" className="sr-only">規則選擇</h2>{choiceCommands.map((command) => <button key={`${command.executionId}:${command.choiceId}:${command.optionId}`} type="button" onClick={() => submitAndClear(command)}>選擇：{command.optionId}</button>)}</div> : null}<button ref={endPhaseRef} data-testid="end-phase" className="primary" type="button" disabled={!endPhaseCommand} aria-describedby="interaction-hint" onClick={() => endPhaseCommand && submitAndClear(endPhaseCommand)}>結束{phaseNames[view.phase]}階段</button>{equipmentCardId ? <button type="button" onClick={() => setEquipmentCardId(undefined)}>取消配戴</button> : null}<button type="button" onClick={restart}>重新開始</button></section>
    ...
    <section className="replay-runner" data-testid="replay-runner"><h2>Replay 診斷</h2><p>貼上 versioned Replay JSON；執行不會修改目前對局或本機存檔。</p><label className="field-label" htmlFor="replay-source">Replay JSON</label><textarea id="replay-source" value={replaySource} onChange={(event) => setReplaySource(event.target.value)} placeholder="貼上 ReplayBundle JSON" /><div className="controls"><button type="button" onClick={() => { const exported = loadCurrentReplay(); if (exported) setReplaySource(exported); }}>載入目前 Replay</button><button data-testid="run-replay" type="button" onClick={() => runReplay(replaySource)}>執行 Replay</button>{replayReport ? <button type="button" onClick={clearReplayReport}>清除結果</button> : null}</div>{replayReport ? <output data-testid="replay-report" role="status" aria-live="polite" aria-atomic="true" className={replayReport.status === 'completed' ? 'replay-success' : 'replay-failure'}>{replayReport.status === 'completed' ? <><strong>{replayReport.message}</strong><span>commands {replayReport.commandCount} · events {replayReport.eventCount} · revision {replayReport.revision}</span></> : <><strong>{replayReport.reasonCode ?? 'MALFORMED_REPLAY'}：{replayReport.message}</strong>{replayReport.commandIndex !== undefined ? <span>command #{replayReport.commandIndex + 1}{replayReport.commandId ? ` (${replayReport.commandId})` : ''}</span> : null}{replayReport.expectedRevision !== undefined ? <span>revision expected {replayReport.expectedRevision} / actual {replayReport.actualRevision}</span> : null}{replayReport.divergence ? <span>first divergence {replayReport.divergence.path}：expected {JSON.stringify(replayReport.divergence.expected)} / actual {JSON.stringify(replayReport.divergence.actual)}</span> : null}</>}</output> : null}</section>
  </main>;
}
