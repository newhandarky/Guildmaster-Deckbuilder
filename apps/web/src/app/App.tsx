...
export function App() {
  ...
  return <main className="app-shell" data-testid="game-app">
    ...
    <section data-testid="hand"><h2>手牌</h2><div className="card-row">{view.self.hand.map((cardId) => {
      ...
      return <Card key={cardId} testId={`hand-card-${cardId}`} instance={view.cards[cardId]} definition={definition as CardDefinition | undefined} onClick={onClick} selected={equipmentCardId === cardId} label={canEquip ? '點選後選隊員' : undefined} />;
    })}</div></section>
