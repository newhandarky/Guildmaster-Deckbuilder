...
export function App() {
  ...
  const interactionHint = !canAct ? '此對局目前不可操作。' : view.activePlayerId !== view.viewerId ? 'AI 正在行動，請等待你的回合。' : isAction ? '可操作手牌會以可點擊狀態顯示。' : `目前是${phaseNames[view.phase]}階段，行動手牌暫不可用。`;
  ...
  return <main className="app-shell" data-testid="game-app">
    ...
    <PartyPanel player={view.self} partyLimit={view.partyLimit} definitions={ruleset.registry.definitions} cardDefinitions={cardDefinitions} presentation={presentation} equipCardId={equipmentCardId} onEquip={(adventurerId) => equipmentCardId && submitAndClear({ type: 'EQUIP_ITEM', cardId: equipmentCardId, adventurerId })} />
