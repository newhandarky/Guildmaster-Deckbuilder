...
type Props = { instance?: CardInstance | undefined; definition?: CardDefinition | undefined; onClick?: (() => void) | undefined; selected?: boolean | undefined; label?: string | undefined };

export function Card({ instance, definition, onClick, selected = false, label }: Props) {
  const disabled = !onClick;
  return <button type="button" className={`card ${definition?.type ?? 'unknown'} ${selected ? 'selected' : ''}`} disabled={disabled} onClick={onClick}>
