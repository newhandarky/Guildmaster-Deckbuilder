...
function view(overrides: Partial<PlayerView> = {}): PlayerView {
  return {
    ...
    partyLimit: 5,
    opponents: [{ id: 'p2', name: '同伴', handCount: 0, partyCount: 0, discardCount: 0, defeatedBosses: 0, counters: [] }],
    zones: {},
    enemyTargets: {},
    cards: {},
    ...overrides,
...
describe('lifecycle interaction model', () => {
  ...
  it('renders waiting progress without inventing actions', () => {
    const model = buildLifecycleInteractionModel(view({
      ...
      opponents: [
        { id: 'p2', name: '同伴', handCount: 0, partyCount: 0, discardCount: 0, defeatedBosses: 0, counters: [] },
        { id: 'p3', name: '第三位玩家', handCount: 0, partyCount: 0, discardCount: 0, defeatedBosses: 0, counters: [] },
      ],
    }), [], [], defaultLifecycleCopyResolver);
    expect(model).toMatchObject({
      kind: 'waiting',
