...
- 遠征入口可明確選擇預設的原創 Demo，或內部限定的「基礎候選數值測試」模式；後者以 Content Pack 指紋保存／恢復，必須通過 engine `allowProvisionalPlaytest` 閘門，已接入首批七種物資與六項道具效果，並持續警告其餘效果尚未啟用。
...
- 預設 runtime 仍是可玩的**原創示範內容包**；另有明確 opt-in 的 provisional foundation pack，接入 30 個中性名稱候選（含首批七種道具／裝備），但不是完整、逐字對應的官方卡表，也不會自動升為 production 內容。候選物資 01／05 可依卡種從棄牌堆取回卡片，04 可棄魔王後抽牌，08「抽 2」、10「棄 1 後抽 2」與 17「抽 3 後棄 1」亦已透過 Effect AST 啟用；選牌效果使用可跨 Snapshot／Replay 恢復的版本化 card-use continuation，其餘卡面效果維持停用。
